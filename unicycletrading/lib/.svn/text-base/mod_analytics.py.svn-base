...
def lift(x, y, buy_threshold=0.0, sell_threshold=0.0, plot=True):
    ...
    meanLiftBuy =  liftBuy[-1] / float(n)
    meanLiftSell = liftSell[-1] / float(n)
    ...
    rawLiftBuy = numpy.insert(numpy.array([liftBuy[i] /
                 float(i) for i in range(1, n)]), 0, 0.0)
    rawLiftSell = numpy.insert(numpy.array([liftSell[i] /
                 float(i) for i in range(1, n)]), 0, 0.0)
    ...
    for i in range(step-1, n, step):
        print '{0:.2f} {1:.3f} {2:.3f} {3:3d}'.format(idx[i],
                                      rawLiftBuy[i],
                                      100.0 * ((rawLiftBuy[i] / meanLiftBuy) - 1.0) / idx[i],
                                      i)
    ...
    for i in range(step-1, n, step):
        print '{0:.2f} {1:.3f} {2:.3f} {3:3d}'.format(idx[i],
                                      rawLiftSell[i],
                                      100.0 * ((rawLiftSell[i] / meanLiftSell) - 1.0) / idx[i],
                                      i)

    # normalize lift by subtracting the cumulative mean lift
    liftBuy -= numpy.array([meanLiftBuy] * n).cumsum()
    liftSell -= numpy.array([meanLiftSell] * n).cumsum()
