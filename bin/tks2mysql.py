...
if files == None:
    print("{0}:[WARNING] No files specified; try: {1} "
          "<config_filenam> <filenames>").format(sys.argv[0], sys.argv[0])
    sys.exit(0)

hi_tks = mod_tks.new(config_filename=cfg)
lo_tks = mod_tks.new(unicycle=hi_tks.get_unicycle())

lores_sample_threshold = hi_tks.get_unicycle().get_dict(
...
for fname in files:
    ...
    raw_fname = fname
    if re.search("\.%s$" % hi_tks.get_config().get("feed", "zip_ext"),
                 fname) is not None:
        os.system("%s %s" % (hi_tks.get_config().get("feed",
                                                     "unzip_command"),
                             fname))
        term_1 = -1 * (len(hi_tks.get_config().get("feed", "zip_ext")) + 1)
        raw_fname = fname[0:term_1]
#        raw_fname = fname[0:(-1 * (len(hi_tks.get_config().get("feed",
#                          "zip_ext")) + 1))]
    ...
    if os.path.getsize(raw_fname) == 0:
        ...
        default_time = hi_tks.get_dbh().get_converted_time(
                       hi_tks.get_config().get("lores", "1day_default_time"),
                       hi_tks.get_config().get("lores", "1day_time_zone"),
                       hi_tks.get_config().get("feed", "time_zone"))

        print("[{0}] {1}").format(index, raw_fname)
