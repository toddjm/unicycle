...
row = unicycle.get_dbh().get_row("SELECT DATE_FORMAT(NOW(), '%Y%m%d'), DATE_FORMAT(NOW(), '%H')")
...
if not (now_base == base):
    ...
    path = "%s/%s/%s/%s" % (os.getenv("TICKS_HOME"),
                         unicycle.get("exchange_dir", unicycle.get("this", "exchange")),
                         "ib",
                         unicycle.get("hires", "interval"))
    # chdir to path
    os.chdir(path)
