...
# load data
#dbh.execute("LOAD DATA LOCAL INFILE '%s' INTO TABLE %s (id, exchange,
# @t0, @t1, currency) SET from_time = CONCAT('1970-01-01 ', @t0),
# to_time = CONCAT('1970-01-01 ', @t1)" % (filename, tblname))
dbh.execute("LOAD DATA LOCAL INFILE '%s' INTO TABLE %s "
            "(id, exchange, @t0, @t1, currency) SET "
            "from_time = CONCAT('1970-01-01 ', @t0), "
            "to_time = CONCAT('1970-01-01 ', @t1)" % (filename, tblname))
