...
class new():
    ...
    def __init__(self,
        ...
        self.unicycle = (unicycle if not unicycle == None
                         else mod_unicycle.new(config_filename))
...
#        left_table = self.getValidSlice().get_tblname() if
#         (left_table == None) else left_table
        return mod_vectorslice.new(dbh=self.dbh, left_table=left_table,
                table=table, where=where, verbose=self.verbose, symbol=symbol)
...
#        if not self.signal_refs.has_key(key):
        if key not in self.signal_refs:
    ...
    def getValidWhereSQL(self):
        ...
        terms.append(
          ...
          % (
          self.get_from_time()))
        terms.append("TIME(ts) < '%s'" % self.get_to_time())
        return ' AND '.join(terms)

    def __setitem__(self, key, value):
        self.signal[key] = np.array(value)

    def __getitem__(self, key):
        if mod_unicycle.is_integer(key):
            return self.signal_list[key]
    ...
    def get_day_mask(self):
        if self.day_mask == None:
            ...
            self.day_mask = mod_signal.argtake(
                    self.day_mask, self.get_valid_mask() == True)
