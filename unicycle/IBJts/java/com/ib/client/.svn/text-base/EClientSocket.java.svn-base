...
public class EClientSocket {
    ...
    public synchronized void eConnect( String host, int port, int clientId) {
	    ...
	    // this must be done before the socket is bound
	...
	m_anyWrapper.message("Server Version:" + m_serverVersion);
        if ( m_serverVersion >= 20 ){
            m_TwsTime = m_reader.readStr();
            m_anyWrapper.message("TWS Time at connection:" + m_TwsTime);
        }
    ...
    public synchronized void eDisconnect() {
        ...
        try {
            // stop reader thread
            if( reader != null) {
		wrapper().message("Interrupt Reader Thread");
                reader.interrupt();
            }
        }
        catch( Exception e) {
