...
parser = argparse.ArgumentParser(description='Request and save resultant futures price data',
...
tks = mod_tks.new(unicycle=unicycle, exchange=unicycle.get_key("exchange_type", "futures"), interval=unicycle.get("hires", "interval"))
...
for date_str in args.dates:
    for ii in range(int(unicycle.get("collect", "try_loop_cnt"))):
        for rec in unicycle.get_dbh().get_all("SELECT id, IFNULL(contract, DATE_FORMAT(last_trade_date, '%s')) AS expiry " % ('%Y%m') +
                                         "FROM %s.futures_roll_dates " % (unicycle.get("mysql", "default_db")) +
                                         "WHERE %s AND " % ("ignore_period=0" if args.ignore else "1=1") +
                                         "%s AND " % ("(%s)" % (' OR '.join(["id='%s'" % (val) for val in args.instrument.split(',')])) if args.instrument else "1=1") +
                                         "%s AND " % ("(%s)" % (' OR '.join(["IFNULL(contract, DATE_FORMAT(last_trade_date, '%s'))='%s'" % ('%Y%m', val) for val in args.expiry.split(',')])) if args.expiry else "1=1") +
                                         "DATE(last_trade_date) >= '%s' AND " % (date_str) +
                                         "DATE(IF(first_trade_date, first_trade_date, '%s')) <= '%s' " % (unicycle.get("config","default_futures_first_trade_date"), date_str) +
                                         "ORDER BY expiry, id"):
